...
from processors.dask_processing import \
    calculate_result as dask_calculate_result
from processors.pandas_processing import \
    calculate_result as pandas_calculate_result
from processors.polars_processing import \
...
assert (
    BENCH_SIZE in BENCH_SIZES
), f"BENCH_SIZE environment variable must be one of: {BENCH_SIZES}"
        ...
        if filename.endswith(".json"):
            with open(os.path.join("outputs", filename), "r") as f:
                ...
                if expected_output == {}:
                    ...
                    raise ValueError(
                        f"Output file {filename} does not match expected output"
                    )
...
def main():
    ...
    if not os.path.exists(RESOURCE_PATH):
        ...
        generate_csv_data(
            num_rows=MEDIUM_DATASET_ROWS, filename=MEDIUM_DATASET_FILENAME
        )
    ...
    small_benchmarks = {
        "pandas_small": lambda: pandas_calculate_result(
            RESOURCE_PATH + SMALL_DATASET_FILENAME
        ),
        "dask_small": lambda: dask_calculate_result(
            RESOURCE_PATH + SMALL_DATASET_FILENAME
        ),
        "polars_small": lambda: polars_calculate_result(
            RESOURCE_PATH + SMALL_DATASET_FILENAME
        ),
    ...
    medium_benchmarks = {
        "pandas_medium": lambda: pandas_calculate_result(
            RESOURCE_PATH + MEDIUM_DATASET_FILENAME
        ),
        "dask_medium": lambda: dask_calculate_result(
            RESOURCE_PATH + MEDIUM_DATASET_FILENAME
        ),
        "polars_medium": lambda: polars_calculate_result(
            RESOURCE_PATH + MEDIUM_DATASET_FILENAME
        ),
    ...
    large_benchmarks = {
        "pandas_large": lambda: pandas_calculate_result(
            RESOURCE_PATH + LARGE_DATASET_FILENAME
        ),
        "dask_large": lambda: dask_calculate_result(
            RESOURCE_PATH + LARGE_DATASET_FILENAME
        ),
        "polars_large": lambda: polars_calculate_result(
            RESOURCE_PATH + LARGE_DATASET_FILENAME
        ),
